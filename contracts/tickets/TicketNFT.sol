...
contract TicketNFT {
    ...
    mapping(bytes32 => mapping(address => uint256)) mappingCategoryUserTixNum;
    // track ticket tokens
    ...
    event TransferredOwnership(bytes32 ticketId, address newOwner);
    ...
    function mintTicket(
        ...
        bytes32 ticketTokenID = keccak256(
            abi.encodePacked(eventID, ticketCategoryID, owner, block.timestamp)
        );
    ...
    function purchaseTicket(
        ...
        (, , , , uint256 remaining, , , uint256 maxTixPerUser) = ticketFactory
            .getTicketCategory(ticketCategoryID);

        // Checks before issuing tickets
        require(
            mappingCategoryUserTixNum[ticketCategoryID][buyer] +
                numTicketsPurchased <=
                maxTixPerUser,
            "Max purchase limit for user reached"
        );
        require(
            remaining >= numTicketsPurchased,
    ...
    function transferOwnership(
        bytes32 ticketId,
        address newOwner
    ) public ownerOnly(ticketId) validTicketId(ticketId) {
    ...
    function transferOwnershipResell(
        bytes32 ticketId,
        address newOwner
    ) public validTicketId(ticketId) {
        tickets[ticketId].prevOwner = tickets[ticketId].owner;
        tickets[ticketId].owner = newOwner;
        emit TransferredOwnership(ticketId, newOwner);
    }
    ...
    function getTicketEvent(
        bytes32 ticketId
    ) public view validTicketId(ticketId) returns (bytes32) {
